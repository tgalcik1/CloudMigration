...
let block_no=1;
let max_break = 4;
let userid=sessionStorage.getItem("username");// This gives the UserID  and stores it during the entire session.

//let num_rep_main = 5;
let num_rep_main = 1;
let num_rep_practice = 1;
let array_stimcond_main = [5,6,7,8,9,5,6,7,8,9,5,6,7,8,9,5,6,7,8,9]; //Experimental condition.
let array_stimcond_practice = [3,4,5]; //Experimental condition.
let time_stimduration_main = 500; //in ms Green & Bavelier (2006)
let time_stimduration_main2 = 250;
let time_stimduration_main3 = 50;
...
//Let the difficulty be an empty string and then a value is assigned to it .
let difficulty = " ";
let difficulty1="Easy";
let difficulty2="Medium";
let difficulty3="Hard";
let timeDurations = [time_stimduration_main, time_stimduration_main2, time_stimduration_main3];
shuffleArray(timeDurations);  //shuffle to randomize the blocks
...
//skip button;
// This allows the user to skip after few trials.
let size_skip_w = Math.round(2.5*ppd);// in pixel
let size_skip_h = Math.round(1.5*ppd); // in pixel
let x_skip =Pos.center_x +  size_skip_w*3; // in pixel
let y_skip = Pos.center_y  + (size_skip_h*6);// in pixel 
////
