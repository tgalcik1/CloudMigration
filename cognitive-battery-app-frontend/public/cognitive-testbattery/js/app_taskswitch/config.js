...
let time_comp_temp=3000;
let time_stimduration = 3000; //in ms
let time_stimduration2 = 2000;
let time_stimduration3 = 1000;
//let time_maskduration = 1000; //in ms
let time_maskduration = 0; //in ms
let time_fixation = 650; // in millisecond

//To randomise the blocks the below code is used
let timeDurations = [time_stimduration, time_stimduration2, time_stimduration3];
...
//skip button;
let size_skip_w = Math.round(2.5*ppd);
let size_skip_h = Math.round(1.5*ppd);
//let x_skip = Pos.center_x + Math.round(2 * ppd) - size_skip_w; // in pixels
//let y_skip = Pos.center_y - Math.round(2 * ppd) + (size_skip_h / 2); // in pixels
//let size_skip_text = Math.round(0.5*ppd);

// set the size of the skip button
//let size_skip_w = 60; // in pixels
//let size_skip_h = 30; // in pixels

// calculate the position of the skip button in pixels
let x_skip =Pos.center_x +  size_skip_w*3;
let y_skip = Pos.center_y  + (size_skip_h*6.5);
