class TimeManager{
    ...
    show(){
        if(Time.scene==0){
          ...
          // if (this.scene >= this.scene_mainstart && this.scene <= this.end_scene) {
          //   show_button_skip();
          // }
    }
      ...
      if (Params.flag_block ==true){
        ...
        if (flag_practice==true){
          ...
          if (flag_break==true){
            ...
            window.parent.postMessage({ command: 'break-survey', task: 'Descartes_TS'}, '*');
            this.scene = this.scene_break;
            button_start.show();
          }else{
            // send a message to parent window indicating that we should display the post-game survey
            window.parent.postMessage({ command: 'end-survey', task: 'Descartes_TS'}, '*');
