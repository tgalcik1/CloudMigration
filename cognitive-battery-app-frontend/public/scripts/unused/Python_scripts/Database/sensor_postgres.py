...
for index,file_path in enumerate(list_file):
    ...
    df['ecg_data']=df['ecg_data'].fillna("None")
    df['eda_data']=df['eda_data'].fillna("None")
    ...
    # Print the JSON data
    cursor.execute("""INSERT INTO sensor_data (subject_id,data,game_type)VALUES (%s, %s,%s);""", (subject_id,json_data,game_type))
